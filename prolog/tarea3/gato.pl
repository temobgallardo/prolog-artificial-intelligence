...
min([[X|Y]], Tablero, (Pos, Val)):-
    actualizar(X,o, Tablero,TableroSig), write('min - actualizar'), nl,
    (
        (
            victoria(TableroSig,o), Pos = X, Val = 1;
            empate(TableroSig), Pos = X, Val = 0
        ); write('min - victory'), nl,
        ultimo(Y,U), write('min - ultimo'), nl,
        ramas(Y,U,RamasY), write('min - ramas'), nl,                    
        max(RamasY, TableroSig, (Pos,Val)), write('min - max'), nl
    ),!.  

min([[X|Y]|T],Tablero,(Pos,Val)):-
    actualizar(X,o, Tablero,TableroSig), write('min2 - actualizar'), nl,   %Genera un tablero con una posicion tentativa
    (
        (
            victoria(TableroSig,o), Pos = X, Val = 1; %Se da un valor para cada tablero final
            empate(TableroSig), Pos = X, Val = 0
        ); write('min2 - victoria'), nl,
        ultimo(Y, U), write('min2 - ultimo'), nl,                           %Termina recursión si llegamos a la hoja
        ramas(Y,U,RamasY), write('min2 - ramas'), nl,                      %Calculan el resto de posiciones segun el movimiento tentativo
        max(RamasY, TableroSig, (PosO,ValO)), write('max'), nl
    ),
    (
        (ValO == -1, Val = ValO, Pos = PosO); write('min2 - assingment'), nl, %Se decide si la posición tentativa es o no llega a un resultado deseado para min
        min(T, Tablero, (Pos,Val)) ,write('min'), nl %Se llama a Max para revisar el siguente nivel
    ),!.          

max([[X|Y]], Tablero, (Pos, Val)):-
    actualizar(X,x, Tablero,TableroSig), write('max - actualizar'), nl,
    (
        (
            victoria(TableroSig,x), Pos = X, Val = -1; 
            empate(TableroSig), Pos = X, Val = 0
        ); write('max - victory'), nl,
        ultimo(Y,U), write('max - ultimo'), nl,
        ramas(Y,U,RamasY), write('max - ramas'), nl,
        min(RamasY, TableroSig, (Pos,Val)), write('min'), nl
    ),!.    

max([[X|Y]|T],Tablero,(Pos,Val)):-
    actualizar(X,x, Tablero,TableroSig),  write('max2 - actualizar'), nl,      %Genera un tablero con una posicion tentativa
    (
        (
            victoria(TableroSig,x), Pos = X, Val = -1;  %Se da un valor para cada tablero final
            empate(TableroSig), Pos = X, Val = 0
        ); write('max2 - victoria'), nl,  
        ultimo(Y, U), write('max2 - ultima'), nl,  
        ramas(Y,U,RamasY),  write('max2 - ramas'), nl,                  %Calculan el resto de posiciones segun el movimiento tentativo
        min(RamasY, TableroSig, (PosX,ValX)), write('min'), nl
    ),
    (
        (ValX == 1, Val = ValX, Pos = PosX); write('max2 - assignment'), nl,    %Se decide si la posición tentativa es o no llega a un resultado deseado para min
